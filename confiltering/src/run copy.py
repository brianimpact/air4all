...
def run(args):
    ...
    for name in tqdm(file_list):
        ...
        if os.path.exists(f'{args.temp_dir}/category_vocabulary/{file_su_name}_category_vocab.pt') == False:
            ...
            #filtering
            su_names, relevant_idx, category_vocab, cal_freq = Filtering(data,args.temp_dir,included_abb,
            ...
            #saving results
            extract_relevant_idx(args.out_path,args.transcript_path,file_su_name,su_names,relevant_idx,cal_freq,included_abb,num_word_threshold=args.num_word_threshold,low_frequency=args.low_frequency)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--temp_dir',default='/home/seokilee/air4all/confiltering/dataset',help='path of saving the results')
    parser.add_argument('--raw_transcript_path',default= '/data5/air4all/transcripts',help='path of raw youtube transcripts')
    parser.add_argument('--raw_doc_transcript_path',default= '/data5/air4all/document_transcripts',help='path of raw manually collected document transcripts')
    parser.add_argument('--transcript_path',default='/data5/air4all/processed_transcripts',help='path of the preprocessed transcripts')
    parser.add_argument('--abb_path',default='/home/seokilee/air4all/confiltering/dataset/abbreviation_list.xlsx',help='load study unit"s abbreviation excel file which should be defined before run')
    ...
    parser.add_argument('--low_frequency',default=0.001,help='criteria for filtering out lower-frequency')
    ...
    parser.add_argument('--truncated_len',default=1,help='length that documents are padded/truncated to, one unit means 512 length of tokens, 100 -> 512*100')
