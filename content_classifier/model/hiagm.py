...
class HiAGM(nn.Module):
    def __init__(self, config, label_ids):
        ...
        if (self.config.model.embedding.dimension != self.config.model.rnn.in_dimension) or self.config.model.embedding.additional_layer:
            self.embedding_layer = nn.Linear(in_features=self.config.model.embedding.dimension,
        ...
        self.text_encoder_rnn = nn.GRU(input_size=self.config.model.rnn.in_dimension,
        ...
        for kernel in self.config.model.cnn.kernels:
            self.text_encoder_cnns.append(torch.nn.Conv1d(in_channels=self.config.model.rnn.out_dimension * 2 if self.config.model.rnn.bidirectional else self.config.model.rnn.out_dimension,
    ...
    def forward(self, batch):
        ...
        cnn_output = []
        for cnn in self.text_encoder_cnns:
            out = cnn(rnn_output)
            out = F.relu(out) # B*D(CNN)*L
